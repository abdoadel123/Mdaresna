...
class StudentModal extends Component {
    ...
    static navigationOptions = {
        ...
        drawerIcon:(<Image  resizeMode='contain' source={noteIcon} style={{ width: 25, height: 25 }} />)
    }
    state = {
        students: []
    }
    selectedStud = key => {
        this.props.navigation.navigate('note_teacher_view', this.state.students[key])
    }
    componentDidMount() {
            ...
            .then((res) => {
                res.map((l) => {
                    ...
                    return fetch(ip + '/teacher/getStud/' + level + '/' + classNum)
                        .then((res) => res.json())
                        .then((res) => {
                            window.notelevel = level;
                            window.noteclass = classNum;
